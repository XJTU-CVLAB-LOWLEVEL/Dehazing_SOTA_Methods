...
% sub-folder "images":
image_path ='E:/DataSet/VideoHazy_v3/Test/image/images2';
filepaths = dir(fullfile(image_path, '*.JPG'));
save_root ='D:/CV_PAPER_PROJECT/DH\results/no-lacal-results'
for n=1:284
    image_name=fullfile(image_path,filepaths(n).name);
    img_hazy = imread(image_name);
    fid = fopen('images/pumpkins_params.txt','r');
    [C] = textscan(fid,'%s %f');
    fclose(fid);
    gamma = C{2}(1);
    A = reshape(estimate_airlight(im2double(img_hazy).^(gamma)),1,1,3);
    [img_dehazed, trans_refined] = non_local_dehazing(img_hazy, A, gamma );
    imwrite(img_dehazed,fullfile(save_root, filepaths(n).name))
    disp(n)
end
% image_name = '100001'; % 'train'; % 'cityscape'; % 'forest'; % 
% img_hazy = imread(['images/',image_name,'_input.JPG']);
