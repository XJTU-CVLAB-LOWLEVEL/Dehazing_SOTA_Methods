...
from datasets.dataset_test1 import Video_dataset_train, Video_dataset_test
...
parser.add_argument("--checkpoint", default='/home/lry/models/VHDTrain/VHDepoch20.pkl', type=str, help="Test on intermediate pkl (default: none)")
...
def test(test_gen, model, criterion):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(test_gen, 1):
            ...
            gt_dir = batch[3]
            gt_dir = "".join(gt_dir)
            HAZYY = batch[4].to(device)

            HAZYY_3 = HAZYY[:, 6:9]
            HAZYY_3 = torch.squeeze(HAZYY_3)
            HAZYY1 = HAZYY_3
            HAZYY_3 = HAZYY_3.permute(1, 2, 0)  # .cpu()
            HAZYY_3 = HAZYY_3.numpy()
...
save_dir = join(root_val_dir, 'Results1')  #--------------------------SR results save path
