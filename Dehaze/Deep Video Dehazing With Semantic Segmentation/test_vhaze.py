...
parser.add_argument("--warped", default=0, type=int, help="Ways of Alignment, 0: No Alignment / 1: Input Alignment / 2: Feature Alignment")
...
training_settings = [
    # {'nEpochs': 5, 'lr': 1e-5, 'step': 22, 'lr_decay': 0.1, 'lambda_db': 0.5, 'warped': True, 'gated': True},
    # {'nEpochs': 100, 'lr': 1e-4, 'step': 6, 'lr_decay': 0.8, 'lambda_db': 0.5, 'tof': False, 'batchSize': 6}
{'nEpochs': 1, 'lr': 5e-5, 'step': 6, 'lr_decay': 0.5, 'lambda_db': 0.5, 'tof': False, 'batchSize': 6},
    # {'nEpochs': 20, 'lr': 2e-5, 'step': 4, 'lr_decay': 0.5, 'lambda_db': 0.1, 'tof': True,
    #  'batchSize': 8}
]
...
def checkpoint(step, epoch, model, moduleNetwork):
    ...
    if opt.tof:
        model_out_path = join(models_folder, "network-finetuned.pytorch")
...
def test(test_gen, model, criterion):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(test_gen, 1):
            ...
            # Hazy = F.interpolate(Hazy, size=(1800, 2708), mode='bicubic', align_corners=False)
            # GT = F.interpolate(GT, size=(1800, 2708), mode='bicubic', align_corners=False)
