...
from datasets.dataset_video import Video_dataset_train, Video_dataset_test
...
parser.add_argument('--dataset', default="/home/lry/video_dehazing/datasets/VideoHazy_v2_re/Train", type=str, help='Path of the training dataset(.h5)')
parser.add_argument('--dataset_test', default="/home/lry/video_dehazing/datasets/VideoHazy_v2_re/Test", type=str, help='Path of the training dataset(.h5)')
...
parser.add_argument('--name', default='VDH11', type=str, help='Filename of the training models')
...
def checkpoint(step, epoch, model, moduleNetwork):
    ...
    models_folder = join(root_folder, 'models')
    models_folder = join(models_folder, opt.name)
    model_out_path = join(models_folder, "VHD1-epoch{:02d}.pkl".format(epoch))
...
def train(train_gen, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_gen, 1):
       ...
       # Flow = batch[1].to(device)
        ...
        Hazy1 = batch[3].to(device)
        HAZY_13 = Hazy1[:, 6:9]
        #vl.show_image(HAZY_13)
        ...
        if opt.warped == 0:
            ...
            # cv2.normalize(Dehazed_32, Dehazed_32, 0, 1, cv2.NORM_MINMAX)
            ...
            for i in range(a):
                    ...
                    # TUy1 = HAZY_13[i:i + 1]
                    # TUy1 = torch.squeeze(TUy1).cpu()
                    # TUy1 = (TUy1.permute(1, 2, 0)).numpy()
                    DehazedY=Dehazed_32[i:i+1]
                    DehazedY = torch.squeeze(DehazedY).cpu()
                    DehazedY= DehazedY.detach().numpy()
                    cv2.normalize(DehazedY, DehazedY, 0, 0.5, cv2.NORM_MINMAX)
                    # plt.imshow(DehazedY)
                    # plt.show()
                    Repair = Repair2(TUy, DehazedY)
                    # plt.imshow(Repair)
                    # plt.show()
                    # Repair1 = Repair2(TUy, DehazedY)
                    # plt.imshow(Repair1)
                    # plt.show()
...
def test(test_gen, model, criterion):
    ...
    with torch.no_grad():
        for iteration, batch in enumerate(test_gen, 1):
            ...
            if opt.warped != 0:
                # warp the inputs
                Hazy_Warped= input_warping(Hazy)
    ...
    for i in range(opt.start_training_step, 5):
        ...
        for epoch in range(opt.start_epoch, opt.nEpochs + 1):
            ...
            for j in range(len(train_sets)):
                    sub_input_dir = os.path.join(input_dir, train_sets[j])
                    sub_gt_dir = os.path.join(gt_dir, train_sets[j])
                    sub_seg_dir = os.path.join(seg_dir,train_sets[j])
                    sub_input_dir1=sub_input_dir
                   # sub_seg_dir = os.path.join(seg_dir, train_sets[j])
                    ...
                    train_set = Video_dataset_train(sub_input_dir,
                                         sub_gt_dir ,sub_seg_dir,sub_input_dir1,cropSize=opt.cropSize,frames=opt.frames)
            ...
            for j in range(len(test_sets)):
                for k in range(2):
                    sub_input_dir_test = os.path.join(input_dir_test, test_sets[j])
                    sub_gt_dir_test = os.path.join(gt_dir_test, test_sets[j])
                    sub_seg_dir_test= os.path.join(seg_dir_test, test_sets[j])
                    print("Folder {}: Input folder is {},segment folder is {}, GT folder is {}".format(j+1, join(sub_input_dir_test, test_sets[j]),
                                                                                                       join(sub_seg_dir_test,test_sets[j]),
                                                                                  join(sub_gt_dir_test, test_sets[j])))
