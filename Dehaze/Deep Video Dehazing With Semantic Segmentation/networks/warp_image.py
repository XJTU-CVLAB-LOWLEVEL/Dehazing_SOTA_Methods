...
try:
	...
	sys.path.insert(0, 'networks/correlation'); import correlation # you should consider upgrading python
# end
...
def Backward(tensorInput, tensorFlow):
	...
	try:
		tensorOutput = torch.nn.functional.grid_sample(input=tensorInput, grid=(Backward_tensorGrid[str(tensorFlow.size())].cuda(tensorInput.device) + tensorFlow).permute(0, 2, 3, 1), mode='bilinear', padding_mode='zeros')
	except:
		print(tensorInput.device)
		print(Backward_tensorGrid[str(tensorFlow.size())].device)
		print(tensorFlow.device)
	tensorMask = tensorOutput[:, -1:, :, :]; tensorMask[tensorMask > 0.999] = 1.0; tensorMask[tensorMask < 1.0] = 0.0

	return tensorOutput[:, :-1, :, :] * tensorMask
# end

def Backward_large(tensorInput, tensorFlow):
	if str(tensorFlow.size()) not in Backward_tensorGrid:
		tensorHorizontal = torch.linspace(-1.0, 1.0, tensorFlow.size(3)*4).view(1, 1, 1, tensorFlow.size(3)*4).expand(tensorFlow.size(0), -1, tensorFlow.size(2)*4, -1)
		tensorVertical = torch.linspace(-1.0, 1.0, tensorFlow.size(2)*4).view(1, 1, tensorFlow.size(2)*4, 1).expand(tensorFlow.size(0), -1, -1, tensorFlow.size(3)*4)
	...
	tensorFlow1 = torch.nn.functional.interpolate(tensorFlow, scale_factor=4, mode='bicubic', align_corners=False)
	# tensorFlow[:, 0, :, :] *= 4
	# tensorFlow[:, 1, :, :] *= 4
	try:
		tensorOutput = torch.nn.functional.grid_sample(input=tensorInput, grid=(Backward_tensorGrid[str(tensorFlow.size())].cuda(tensorInput.device) + tensorFlow1).permute(0, 2, 3, 1), mode='bilinear', padding_mode='zeros')
	except:
		print(tensorInput.device)
		print(Backward_tensorGrid[str(tensorFlow.size())].device)
		print(tensorFlow.device)
...
def estimate(moduleNetwork, tensorFirst, tensorSecond):
	...
	if True:
		tensorPreprocessedFirst = tensorFirst[:, [2,1,0]]
		tensorPreprocessedSecond = tensorSecond[:, [2,1,0]]
		...
		### Try to warp the image ###
		# NOTE:
		# 1. tensorPreprocessedSecond has different size with origin second image
		# 2. the image tensor in PWC-NET is BGR, so need to revert to RGB before warping
		# tensorFlow = torch.nn.functional.interpolate(tensorFlow, scale_factor=4, mode='bicubic', align_corners=False)
		# tensorFlow[:, 0, :, :] *= 4
		# tensorFlow[:, 1, :, :] *= 4
		tensorSecond_warped = Backward(tensorInput=tensorSecond, tensorFlow=tensorFlow)

		# ImageFirst = transforms.ToPILImage()(tensorFirst.cpu()[0])
		# ImageFirst.save('images/Image_Anchor_1.png')
		# ImageSecond = transforms.ToPILImage()(tensorSecond.cpu()[0])
		# ImageSecond.save('images/Image_Second_1.png')
		# warpedImageSecond = transforms.ToPILImage()(tensorSecond_warped.cpu()[0])
		# warpedImageSecond.save('images/Warped_Image_Second_1.png')
        #
		# ImageFirst = transforms.ToPILImage()(tensorFirst.cpu()[1])
		# ImageFirst.save('images/Image_Anchor_2.png')
		# ImageSecond = transforms.ToPILImage()(tensorSecond.cpu()[1])
		# ImageSecond.save('images/Image_Second_2.png')
		# warpedImageSecond = transforms.ToPILImage()(tensorSecond_warped.cpu()[1])
		# warpedImageSecond.save('images/Warped_Image_Second_2.png')


		tensorOutput = tensorFlow

	# end

	return tensorOutput, tensorSecond_warped
# end
