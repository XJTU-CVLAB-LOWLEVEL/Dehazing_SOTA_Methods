# Inspired by RBPN, can use train the no_tof or tof versions
# Next, it can be modified to feature warping by:
# ft = self.conv_input(x) and ft1 = ft[:, 0:16], then ft1 = self.fuse31_1(cat(ft1, ft3, F1)).
# Finally, the F1-F5 can be calculated from ft1-ft5 by using the modified PWC_feature
...
class Net(nn.Module):
    def __init__(self, opt, res_blocks=18):
        ...
        self.fuse1 = ConvLayer(8, 16, kernel_size=3, stride=1)
        # self.fuse2 = ResidualBlock(16)
        self.fuse2 = ConvLayer(16*self.frames, 16, kernel_size=1, stride=1)
    ...
    def forward(self, x, flowmaps):
        ...
        ft1 = self.fuse1(torch.cat((I1, F1, I3), dim=1))
        ft2 = self.fuse1(torch.cat((I2, F2, I3), dim=1))
        ft3 = self.conv_input(I3)
        ft4 = self.fuse1(torch.cat((I4, F4, I3), dim=1))
        ft5 = self.fuse1(torch.cat((I5, F5, I3), dim=1))


        res1x = self.fuse2(torch.cat((ft1, ft2, ft3, ft4 ,ft5), dim=1))
