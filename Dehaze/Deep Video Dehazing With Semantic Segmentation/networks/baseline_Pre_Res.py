...
class ResidualBlock_Flat(torch.nn.Module):
    def __init__(self, channels):
        super(ResidualBlock_Flat, self).__init__()
...
class PreNet(nn.Module):
    def __init__(self, res_blocks=1):
        ...
        for i in range(0, res_blocks):
            self.dehaze.add_module('res%d' % i, ResidualBlock_Flat(256))


    def forward(self, x):
        res1x = self.conv_input(x)
        ft = res1x
        out = self.dense0(res1x)
        x = out + res1x

        res2x = self.conv2x(x)
        ft_2x = res2x
        out_2x = self.dense1(res2x)
        res2x = out_2x + res2x


        res4x =self.conv4x(res2x)
        ft_4x = res4x
        out_4x = self.dense2(res4x)
        res4x = out_4x + res4x

        res8x = self.conv8x(res4x)
        ft_8x = res8x
        out_8x = self.dense3(res8x)
        res8x = out_8x + res8x


        res16x = self.conv16x(res8x)
        ft_16x = res16x
        out_16x = self.dehaze(res16x)
...
class Net(nn.Module):
    ...
    def forward(self, x, flowmaps):
        ...
        x = x.view(-1, 3, h, w)
        # x1 = x.view(n,-1,h,w)
        [ft, out, ft_2x, out_2x, ft_4x, out_4x, ft_8x, out_8x, ft_16x, out_16x] = self.pre_net(x)

        center = self.frames // 2
        N, C, H, W = out.size()
        ft = ft.view(n, -1, H, W)[:, C*center:C*(center+1)]
        out = out.view(n, -1, H, W)
        N, C2, H2, W2 = out_2x.size()
        ft_2x = ft_2x.view(n, -1, H2, W2)[:, C2*center:C2*(center+1)]
        out_2x = out_2x.view(n, -1, H2, W2)
        N, C4, H4, W4 = out_4x.size()
        ft_4x = ft_4x.view(n, -1, H4, W4)[:, C4*center:C4*(center+1)]
        out_4x = out_4x.view(n, -1, H4, W4)
        N, C8, H8, W8 = out_8x.size()
        ft_8x = ft_8x.view(n, -1, H8, W8)[:, C8*center:C8*(center+1)]
        out_8x = out_8x.view(n, -1, H8, W8)
        N, C16, H16, W16 = out_16x.size()
        ft_16x = ft_16x.view(n, -1, H16, W16)[:, C16*center:C16*(center+1)]
        out_16x = out_16x.view(n, -1, H16, W16)

        if self.warped == 1 or self.warped == 0:
            out = out + ft.repeat(1, self.frames, 1, 1)
            out_2x = out_2x + ft_2x.repeat(1, self.frames, 1, 1)
            out_4x = out_4x + ft_4x.repeat(1, self.frames, 1, 1)
            out_8x = out_8x + ft_8x.repeat(1, self.frames, 1, 1)
            out_16x = out_16x + ft_16x.repeat(1, self.frames, 1, 1)
        ...
        if self.warped == 2:
            ...
            for i in range(self.frames):
                if i == self.frames // 2:
                    base.append(out[:, i * C:(i + 1) * C] + ft)
                    base2x.append(out_2x[:, i * C2:(i + 1) * C2] + ft_2x)
                    base4x.append(out_4x[:, i * C4:(i + 1) * C4] + ft_4x)
                    base8x.append(out_8x[:, i * C8:(i + 1) * C8] + ft_8x)
                    base16x.append(out_16x[:, i * C16:(i + 1) * C16] + ft_16x)
                    continue
                base.append(
