...
class RDN(nn.Module):
    ...
    def forward(self, x, flowmaps):
        ...
        flowmaps_LR = F.interpolate(flowmaps, scale_factor=0.25, mode='bilinear', align_corners=False)
        flowmaps_LR = flowmaps_LR / 4
        h = h // 4
        w = w // 4
        ...
        for i in range(self.D//2):
            ...
            for i in range(frames):
                if i == frames // 2:
                    ...
                    continue
                inputs_warp.append(Backward(tensorInput=inputs[:,:, i], tensorFlow=flowmaps_LR[:, i*2:(i+1)*2]).view(n,64,1,h,w))
