...
class DataValSetV(data.Dataset):
    def __init__(self, root_dir, flow_dir, isReal= False, frames=5):
        #one input & ground truth
        self.input_dir = join(root_dir, 'hazy')
        self.sr_dir = join(root_dir, flow_dir)
        print(self.sr_dir)
        self.gt_dir = join(root_dir, 'gt')
        self.isReal = isReal
        self.frames = frames
        ...
        for folder in folders:
            for x in sorted(os.listdir(join(self.input_dir, folder))):
                self.names_in.append(join(folder, x))
        for folder in folders:
            for x in sorted(os.listdir(join(self.gt_dir, folder))):
                self.names_gt.append(join(folder, x))

    def __len__(self):
        return len(self.names_in)

    def __getitem__(self, index):
        ################# For LR_Blur Images #################
        input = []
        HR = []
        name = self.names_gt[index]
        sub_index = index % 100
        scale = index // 100
        indexs = np.clip(range(sub_index - self.frames//2, sub_index + self.frames//2 + 1), 0, 99) + scale*100

        input_imgs = [Image.open(join(self.input_dir, self.names_in[i])) for i in indexs]
        HR_imgs = [Image.open(join(self.sr_dir, self.names_in[i])) for i in indexs]
        ...
        for HR_img in HR_imgs:
            HR_patch = np.array(HR_img, dtype=np.float32) / 255
            HR_patch = HR_patch.transpose((2, 0, 1))
            HR.append(HR_patch)

        input_img = Image.open(os.path.join(self.input_dir, self.names_in[index])).convert('RGB')
        size_in = input_img.size

        if not self.isReal:
        ################# For GT and LR_Deblur Images #################
            gt_img = Image.open(os.path.join(self.gt_dir, self.names_gt[index])).convert('RGB')
...
class DataSetV(data.Dataset):
    def __init__(self, input_dir, sr_dir, gt_dir, frames=5, cropSize=32, sigma=2, gamma=2.2):
        ...
        self.names_in = [x for x in sorted(os.listdir(self.input_dir)) if is_image_file(x)]

    def __len__(self):
        return len(self.names_gt)
    ...
    def __getitem__(self, index):
        ...
        input_imgs = [Image.open(join(self.input_dir, self.names_in[i]))\
                      for i in self.inputlists]
        HR_imgs = [Image.open(join(self.sr_dir, self.names_in[i]))\
                      for i in self.inputlists]
        ...
        for HR_img in HR_imgs:
            ...
            # input_patch.resize(face_size, Image.ANTIALIAS)
            HR_patch = HR_patch.resize((self.cropSize, self.cropSize), Image.BICUBIC)
            HR_patch = np.array(HR_patch, dtype=np.float32) / 255
            HR_patch = HR_patch.transpose((2, 0, 1))
            # randomly flip
            if isFilp == 0:
