...
class Video_dataset_train(data.Dataset):
    def __init__(self, train_dir,gt_dir,seg_dir,train_dir1,cropSize=128, frames=5):
        super(Video_dataset_train, self).__init__()
        self.gt_dir = gt_dir
        self.input_dir = train_dir
        self.input_dir1 = train_dir1
        ...
        self.input1_list = []
        for img in sorted(os.listdir(train_dir1)):
            if is_image_file(img):
                self.input1_list.append(join(train_dir1, img))
    ...
    def __getitem__(self, index):
        ...
        input1_imgs = [Image.open((self.input1_list[i])).convert('RGB') for i in self.input1_frame_lists]
...
class Video_dataset_test(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     input_img = torch.from_numpy(input_img.copy()).float()
        #     inputs.append(input_img)
        # inputs = torch.cat(inputs, 0).reshape(3,256,256)
        # inputs = np.array(inputs, dtype=np.float32)
        for input_img in input_imgs:
            input_patch = input_img.crop(
                (left_top_w, left_top_h, left_top_w + self.fineSize, left_top_h + self.fineSize))
        ...
        for seg_img in seg_imgs:
            ...
            input_patch = torch.from_numpy(input_patch.copy()).float()
            input_patch = torch.unsqueeze(input_patch, 0)
            # nomalize = transforms.Normalize(mean=(0.5), std=(0.5))
            # input_patch = nomalize(input_patch)
            #input_patch = torch.from_numpy(input_patch.copy()).float()
            seg.append(input_patch)
