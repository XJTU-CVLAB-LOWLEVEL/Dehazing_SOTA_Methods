...
class PCDAlignment(nn.Module):
    ...
    def forward(self, nbr_feat_l, ref_feat_l):
        ...
        for i in range(3, 0, -1):
            ...
            if i == 3:
                ...
                offset = self.lrelu(self.offset_conv2[level](torch.cat(
                    [offset, upsampled_offset], dim=1)))#ct Upsampling of the previous layer offset
                offset = self.lrelu(self.offset_conv3[level](offset))

            feat = self.dcn_pack[level](nbr_feat_l[i - 1], offset)#offset+neighboring frames as input of Deformable convolution.
            if i < 3:
        ...
        # Cascading
        offset = torch.cat([feat, ref_feat_l[0]], dim=1)#the part with light purple background in Fig. 3
        offset = self.lrelu(
...
class PredeblurModule(nn.Module):
    ...
    def forward(self, x):
        feat_l1 = self.lrelu(self.conv_first(x))#x.view(-1, 3, 1800, 2708))
        if self.hr_in:
...
class EDVR(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_extraction = make_layer(        #stacked residual blocks
    ...
    def forward(self, x):
        # x = F.interpolate(x, size=(1792, 2704), mode='bicubic', align_corners=False)
        b, t, c, h, w = x.size()
        Hazy = x.view(-1, c, h,w)
        Hazy = F.interpolate(Hazy, size=(1792, 2704), mode='bicubic', align_corners=False)
        t,c,h,w = Hazy.size()
        # x = x.unsqueeze(0)
        ...
        # L1
        if self.with_predeblur:
            feat_l1 = self.conv_1x1(self.predeblur(Hazy))#x.view(-1, c, h, w))
            if self.hr_in:
                h, w = h // 4, w // 4
        else:
            feat_l1 = self.lrelu(self.conv_first(Hazy))#x.view(-1, c, h, w))
        ...
        if self.hr_in:
            base = x_center
            base = F.interpolate(base, size=(1792, 2704), mode='bicubic', align_corners=False)
        else:
