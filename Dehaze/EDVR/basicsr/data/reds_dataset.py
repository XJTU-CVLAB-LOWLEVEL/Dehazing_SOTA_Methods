...
class REDSDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            for line in fin:
                ...
                self.keys.extend(
                    [f'{folder}/{i:5d}' for i in range(int(frame_num))])
    ...
    def __getitem__(self, index):
        ...
        key = self.keys[index]
        clip_name, frame_name = key.split('/')# key example: 000/00000
        center_frame_idx = int(frame_name)

        # subdir = random.randint(1, 10)
        # subdir = str(subdir)
        ...
        with open('/home/yzt/EDVR/basicsr/data/meta_info/data.txt', 'r') as fin:
            for line in fin:
                folder_, frame__num, __ = line.split(' ')
                if clip_name == folder_:
                    frame_n = frame__num
        while (start_frame_idx < 0) or (end_frame_idx > int(frame_n)-1):
            center_frame_idx = random.randint(0, int(frame_n)-1)
            start_frame_idx = (
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = self.lq_root / clip_name / f'{neighbor:05d}.JPG'
        ...
        if self.flow_root is not None:
            ...
            for i in range(self.num_half_frames, 0, -1):
                if self.is_lmdb:
                    ...
                    flow_path = (
                        self.flow_root / clip_name / f'{frame_name}_p{i}.JPG')
